...
import { budgetSchema, calculateBudgetUsage, calculateFinancialHealth, creditProfileSchema, expenseSchema, getOverspendingAlerts, goalSchema, investmentSchema, loginSchema, registerSchema, simulateCreditScore } from "@finsphere/shared";
import { authMiddleware, signToken } from "./auth.js";
import { createSeedStore, makeId } from "./store.js";
export function createApp(store = createSeedStore()) {
    const app = express();
    app.use(helmet());
    ...
    app.post("/api/credit-profile/simulate", requireAuth, (req, res) => {
        ...
        if (existing)
            ...
            store.creditProfiles.push(profile);
...
function updateEntity(req, res, rows, schema, key) {
    const index = rows.findIndex((item) => item.id === req.params.id && item.userId === res.locals.user.id);
    if (index === -1)
        return res.status(404).json({ message: "Not found" });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ message: `Invalid ${key}`, issues: parsed.error.issues });
    rows[index] = { ...rows[index], ...parsed.data };
    return res.json({ [key]: rows[index] });
}
function deleteEntity(req, res, rows) {
    const index = rows.findIndex((item) => item.id === req.params.id && item.userId === res.locals.user.id);
    if (index === -1)
        return res.status(404).json({ message: "Not found" });
    rows.splice(index, 1);
